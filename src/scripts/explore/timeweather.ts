// ─── 시간대 + 날씨 시스템 ───
// 실시간 시계 자동 → sky/light/fog 전환 + 수동 오버라이드 + 비/눈 파티클
...
const DAWN: P = {
    ...
    ambCol: [0.45, 0.35, 0.50], ambI: 0.6,
    hemiSky: [0.60, 0.45, 0.55], hemiGnd: [0.30, 0.25, 0.15], hemiI: 0.4,
    fillCol: [0.50, 0.35, 0.55], fillI: 0.3,
    fogCol: [0.55, 0.40, 0.45], fogD: 0.012,
    starOp: 0.05, exposure: 1.2,
    waterDeep: [0.12, 0.35, 0.40], waterShallow: [0.45, 0.55, 0.50],
};
...
const DAY: P = {
    ...
    ambCol: [0.53, 0.60, 0.73], ambI: 1.2,
    hemiSky: [0.53, 0.73, 1.0], hemiGnd: [0.27, 0.40, 0.20], hemiI: 0.7,
    fillCol: [0.60, 0.67, 0.80], fillI: 0.4,
    fogCol: [0.72, 0.85, 0.94], fogD: 0.008,
    starOp: 0.0, exposure: 1.4,
    waterDeep: [0.10, 0.47, 0.53], waterShallow: [0.35, 0.85, 0.91],
};
...
const SUNSET: P = {
    ...
    ambCol: [0.50, 0.30, 0.28], ambI: 0.7,
    hemiSky: [0.65, 0.35, 0.30], hemiGnd: [0.20, 0.15, 0.10], hemiI: 0.4,
    fillCol: [0.60, 0.30, 0.25], fillI: 0.25,
    fogCol: [0.50, 0.30, 0.25], fogD: 0.011,
    starOp: 0.08, exposure: 1.3,
    waterDeep: [0.15, 0.20, 0.35], waterShallow: [0.55, 0.40, 0.35],
};
...
const NIGHT: P = {
    ...
    ambCol: [0.12, 0.14, 0.25], ambI: 0.35,
    hemiSky: [0.10, 0.12, 0.25], hemiGnd: [0.05, 0.05, 0.08], hemiI: 0.2,
    fillCol: [0.15, 0.18, 0.30], fillI: 0.15,
    fogCol: [0.06, 0.06, 0.12], fogD: 0.014,
    starOp: 0.6, exposure: 0.8,
    waterDeep: [0.04, 0.08, 0.18], waterShallow: [0.10, 0.20, 0.30],
};
...
function lC3(a: C3, b: C3, t: number): C3 {
...
function blendP(a: P, b: P, t: number): P {
    return {
        ...
        sunCol: lC3(a.sunCol, b.sunCol, t), sunI: lN(a.sunI, b.sunI, t),
        sunPos: lC3(a.sunPos, b.sunPos, t) as C3,
        ambCol: lC3(a.ambCol, b.ambCol, t), ambI: lN(a.ambI, b.ambI, t),
        hemiSky: lC3(a.hemiSky, b.hemiSky, t), hemiGnd: lC3(a.hemiGnd, b.hemiGnd, t),
        hemiI: lN(a.hemiI, b.hemiI, t),
...
function applyWeather(p: P, weather: WeatherName): P {
    ...
    if (weather === 'rain') {
        ...
        out.sunI = p.sunI * 0.25;
        out.ambI = p.ambI * 0.65;
        out.hemiI = p.hemiI * 0.5;
        out.fillI = p.fillI * 0.4;
        out.fogCol = lC3(p.fogCol, [0.15, 0.15, 0.18], 0.5);
        out.fogD = p.fogD * 1.8;
        ...
        out.ambCol = lC3(p.ambCol, [0.55, 0.58, 0.65], 0.35);
        out.ambI = p.ambI * 0.85;
        out.hemiI = p.hemiI * 0.7;
        out.fogCol = lC3(p.fogCol, [0.55, 0.58, 0.62], 0.45);
        out.fogD = p.fogD * 1.4;
...
function createRainSystem(scene: THREE.Scene, count: number): {
    group: THREE.LineSegments; pos: Float32Array; vel: Float32Array; update(dt: number): void;
} {
    const pos = new Float32Array(count * 6); // 2 verts per segment
    const vel = new Float32Array(count); // y velocity per drop
    const SPREAD = 80, HEIGHT = 30;

    for (let i = 0; i < count; i++) {
        const x = (Math.random() - 0.5) * SPREAD;
        const y = Math.random() * HEIGHT;
        const z = -29 + (Math.random() - 0.5) * SPREAD;
        const len = 0.3 + Math.random() * 0.4;
        const wind = -0.05;
        pos[i * 6] = x; pos[i * 6 + 1] = y; pos[i * 6 + 2] = z;
        pos[i * 6 + 3] = x + wind; pos[i * 6 + 4] = y - len; pos[i * 6 + 5] = z;
...
function createSnowSystem(scene: THREE.Scene, count: number): {
    group: THREE.Points; pos: Float32Array; drift: Float32Array; update(dt: number, t: number): void;
} {
    const pos = new Float32Array(count * 3);
    const drift = new Float32Array(count * 2); // x,z drift per flake
    const SPREAD = 80, HEIGHT = 25;

    for (let i = 0; i < count; i++) {
        pos[i * 3] = (Math.random() - 0.5) * SPREAD;
        pos[i * 3 + 1] = Math.random() * HEIGHT;
        pos[i * 3 + 2] = -29 + (Math.random() - 0.5) * SPREAD;
    ...
    const mat = new THREE.PointsMaterial({
        color: 0xeef0f8, size: 0.12, transparent: true, opacity: 0.7,
    ...
    return {
        ...
        update(dt, t) {
            for (let i = 0; i < count; i++) {
                ...
                if (pos[i * 3 + 1] < -1) {
                    pos[i * 3] = (Math.random() - 0.5) * SPREAD;
                    pos[i * 3 + 1] = HEIGHT + Math.random() * 3;
                    pos[i * 3 + 2] = -29 + (Math.random() - 0.5) * SPREAD;
                }
            }
            geo.attributes.position.needsUpdate = true;
...
export function createTimeWeather(refs: EnvironmentRefs, isMobile: boolean): TimeWeather {
    ...
    // 현재 적용 중인 preset (매 프레임 lerp 타겟)
    let currentP: P = DAY;
    let targetP: P = DAY;
    let transitionSpeed = 0; // 0 = instant, >0 = lerping

    // 날씨 파티클
    const rainCount = isMobile ? 150 : 350;
    const snowCount = isMobile ? 100 : 250;
    const rain = createRainSystem(refs.scene, rainCount);
    const snow = createSnowSystem(refs.scene, snowCount);

    // 번개 타이머
    let lightningTimer = 10 + Math.random() * 15;
    let lightningFlash = 0;

    // 시간 누적 (ambient animation용)
    let elapsed = 0;

    // ── 초기화: 현재 시각 기반 스냅 ──
    const initHour = new Date().getHours() + new Date().getMinutes() / 60;
    const initBlend = getAutoBlend(initHour);
    currentP = applyWeather(blendP(initBlend.a, initBlend.b, initBlend.t), weather);
    targetP = currentP;
    ...
    function applyPreset(p: P): void {
        ...
        if (refs.scene.background instanceof THREE.Color) {
            setC(refs.scene.background, p.bg);
        }

        // Sun
        setC(refs.sunLight.color, p.sunCol);
        refs.sunLight.intensity = p.sunI;
        refs.sunLight.position.set(p.sunPos[0], p.sunPos[1], p.sunPos[2]);

        // Ambient
        setC(refs.ambientLight.color, p.ambCol);
        refs.ambientLight.intensity = p.ambI;

        // Hemisphere
        setC(refs.hemiLight.color, p.hemiSky);
        setC(refs.hemiLight.groundColor, p.hemiGnd);
        refs.hemiLight.intensity = p.hemiI;

        // Fill
        setC(refs.fillLight.color, p.fillCol);
        refs.fillLight.intensity = p.fillI;

        // Fog
        const fog = refs.scene.fog as THREE.FogExp2;
        if (fog) {
            setC(fog.color, p.fogCol);
            fog.density = p.fogD;
        }
        ...
        if (wMat.uniforms) {
            setC(wMat.uniforms.uDeep.value, p.waterDeep);
            setC(wMat.uniforms.uShallow.value, p.waterShallow);
        }
    }
    ...
    return {
        update(dt) {
            ...
            // 타겟 계산 (auto 모드에서는 매 프레임, manual은 setTime 시에만)
            if (timeMode === 'auto') {
                targetP = computeTarget();
            }

            // Smooth lerp
            const speed = transitionSpeed > 0 ? transitionSpeed : 3.0;
            const t = Math.min(1, speed * dt);
            currentP = blendP(currentP, targetP, t);
            applyPreset(currentP);
            ...
            if (weather === 'rain') {
                ...
                lightningTimer -= dt;
                if (lightningTimer <= 0) {
                    lightningTimer = 8 + Math.random() * 18;
                    if (Math.random() < 0.6) { // 60% 확률로 실제 번개
                        lightningFlash = 0.15;
                    }
                }
                if (lightningFlash > 0) {
                    ...
                    refs.scene.background = new THREE.Color().setRGB(
                        currentP.bg[0] + flash * 0.4,
                        currentP.bg[1] + flash * 0.4,
                        currentP.bg[2] + flash * 0.45,
                    );
                }
            }
            ...
            // 바람에 의한 비 줄기 흔들림
            if (weather === 'rain') {
                (rain.group.material as THREE.LineBasicMaterial).opacity =
                    0.3 + Math.sin(elapsed * 0.5) * 0.08;
            }
        },
        ...
        setTime(name) {
            timeMode = name;
            if (name !== 'auto') {
                targetP = applyWeather(PRESET_MAP[name], weather);
            } else {
                targetP = computeTarget();
            }
            transitionSpeed = 2.0; // 부드러운 전환
        },
        ...
        setWeather(name) {
            ...
            if (name === 'rain') {
                ...
                snow.group.visible = false;
            } else if (name === 'snow') {
                rain.group.visible = false;
                snow.group.visible = true;
            } else {
                rain.group.visible = false;
                snow.group.visible = false;
            }
