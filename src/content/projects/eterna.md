...
NPC 대화에 음성 입력이 필요했는데, Windows·Android·iOS를 한 번에 덮는 SDK가 마땅치 않았고 유료 솔루션은 비용과 라이선스가 부담이었습니다. 외부 의존을 Repository 한 곳으로 최소화한다는 설계 방침과도 맞아 직접 구현을 택했습니다 — Windows는 Whisper, 모바일은 네이티브(Java/ObjC)로 처리하고, 음성 스트리밍과 녹음도 별도 라이브러리 없이 만들었습니다.
