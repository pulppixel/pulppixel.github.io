...
// These need different emissive/metalness per body part type, so they stay local.

/** Costume box - visible color with mild glow */
...
/** Glow accent - dark base, strong emissive (eyes, LEDs) */
...
const FZ = 0.245; // face plane Z offset
...
function addEyes(hd: THREE.Group, spacing = 0.09, size = 0.065, yOff = 0) {
  const eyeL = setPos(facePlane(size, size + 0.01, 0x1a1528), -spacing, yOff, FZ);
  hd.add(eyeL);
  const eyeR = setPos(facePlane(size, size + 0.01, 0x1a1528), spacing, yOff, FZ);
  hd.add(eyeR);
  const hlL = setPos(facePlane(0.025, 0.025, 0xffffff), -spacing + 0.015, yOff + 0.02, FZ + 0.002);
  hd.add(hlL);
  const hlR = setPos(facePlane(0.025, 0.025, 0xffffff), spacing + 0.015, yOff + 0.02, FZ + 0.002);
  hd.add(hlR);
...
const buildBunny: Builder = (hd, bd, aL, aR, lL, lR, root) => {
  ...
  bd.add(setPos(BX(0.20, 0.16, 0.01, WH, 0.15), 0, 0, 0.115));
  addArms(aL, aR, PK, SK);
...
const buildFrog: Builder = (hd, bd, aL, aR, lL, lR) => {
  ...
  bd.add(setPos(BX(0.22, 0.16, 0.01, BELLY, 0.15), 0, 0, 0.115));

  const buildArm = (g: THREE.Group) => {
    g.add(setPos(BX(0.12, 0.26, 0.13, GR), 0, -0.13, 0));
    g.add(setPos(BX(0.11, 0.06, 0.11, DK, 0.2), 0, -0.30, 0));
  };
  buildArm(aL); buildArm(aR);
...
const buildBear: Builder = (hd, bd, aL, aR, lL, lR, root) => {
  ...
  bd.add(setPos(BX(0.20, 0.16, 0.01, CREAM, 0.15), 0, 0, 0.115));
  addArms(aL, aR, BR, BR);
...
const buildRobot: Builder = (hd, bd, aL, aR, lL, lR) => {
  ...
  const buildArm = (g: THREE.Group) => {
    g.add(setPos(BX(0.12, 0.28, 0.13, GY, 0.12), 0, -0.14, 0));
    g.add(setPos(GL(0.08, 0.03, 0.09, TEAL, 0.5), 0, -0.30, 0));
  };
  buildArm(aL); buildArm(aR);
...
const buildPenguin: Builder = (hd, bd, aL, aR, lL, lR, root) => {
  ...
  bd.add(setPos(BX(0.20, 0.22, 0.01, WH, 0.15), 0, 0, 0.115));
