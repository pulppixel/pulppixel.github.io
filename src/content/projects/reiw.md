...
IW Zombie에서 팀 최초로 도입했던 Tencent IM 채팅을, 약 40명 규모의 메타버스 프로젝트 REIW로 가져와야 했습니다. 그대로 옮겨 쓸 수는 없었습니다. World/Whisper/Current/Guild/Party/System으로 채널 구조가 훨씬 복잡해졌고, 음성 메시지 송수신과 좋아요 표기 동기화처럼 이전에는 없던 기능이 요구됐습니다. 첫 도입 당시 일정에 맞춰 붙였던 구조의 부채도 그대로 남아 있었습니다. 이식이 아니라 재설계가 필요하다고 판단했습니다.
...
프로토타입 검증을 마치고 폴리싱된 2.0 기획에 맞춰 본제작으로 이어질 예정이었지만, 이후 회사가 문을 닫으면서 실현되지 못했습니다.
...
이 프로젝트에서 VContainer 기반 DI + MVVM을 실제로 적용해 봤는데, 동작 자체는 깔끔했지만 DI 컨테이너의 학습 비용이 팀 전체로 번지고 MVVM과 섞이면서 의존 그래프를 읽기 어려워지는 구간을 관찰했습니다. 여기서 얻은 관점이 이후 ETERNA의 3계층 설계로 이어졌습니다.

NPC 퀘스트 시스템 구현, 서버팀에 FlatBuffers 프로토콜을 정리해 요청하는 작업도 병행했고, REIW와 ETERNA가 병행되던 시기에 저는 ETERNA로 배치를 옮겼습니다.
