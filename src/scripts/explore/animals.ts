// ─── NPC 동물 ───
// 복셀 토끼(FSM) · 새(비행/착지) · 나비(부유) — 월드 생동감
...
function mt(c: number): THREE.MeshStandardMaterial {
    return new THREE.MeshStandardMaterial({ color: c, metalness: 0.05, roughness: 0.85 });
}

function bx(w: number, h: number, d: number, c: number): THREE.Mesh {
    const m = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), mt(c));
    m.castShadow = true;
    return m;
}

function fp(w: number, h: number, c: number): THREE.Mesh {
...
function createRabbit(scene: THREE.Scene, x: number, z: number): Rabbit {
    ...
    const body = at(bx(0.35, 0.28, 0.45, CREAM), 0, 0.28, 0);
    g.add(body);

    const head = new THREE.Group();
    head.position.set(0, 0.45, 0.18);
    head.add(bx(0.28, 0.24, 0.26, CREAM));

    const earL = at(bx(0.07, 0.22, 0.04, CREAM), -0.08, 0.20, -0.02);
    head.add(earL);
    head.add(at(bx(0.04, 0.14, 0.01, RPINK), -0.08, 0.21, 0.005));
    const earR = at(bx(0.07, 0.22, 0.04, CREAM), 0.08, 0.20, -0.02);
    head.add(earR);
    head.add(at(bx(0.04, 0.14, 0.01, RPINK), 0.08, 0.21, 0.005));

    head.add(at(fp(0.04, 0.05, RDARK), -0.08, 0.02, 0.131));
    head.add(at(fp(0.04, 0.05, RDARK), 0.08, 0.02, 0.131));
    head.add(at(fp(0.015, 0.015, RWHITE), -0.065, 0.035, 0.133));
    head.add(at(fp(0.015, 0.015, RWHITE), 0.065, 0.035, 0.133));
    head.add(at(fp(0.03, 0.02, RPINK), 0, -0.04, 0.131));
    g.add(head);

    const legFL = at(bx(0.08, 0.14, 0.08, CREAM), -0.10, 0.07, 0.14);
    const legFR = at(bx(0.08, 0.14, 0.08, CREAM), 0.10, 0.07, 0.14);
    const legBL = at(bx(0.09, 0.12, 0.12, CREAM), -0.10, 0.06, -0.14);
    const legBR = at(bx(0.09, 0.12, 0.12, CREAM), 0.10, 0.06, -0.14);
    g.add(legFL, legFR, legBL, legBR);

    const tail = at(bx(0.09, 0.09, 0.06, RWHITE), 0, 0.28, -0.26);
    g.add(tail);
...
function updateRabbit(r: Rabbit, dt: number, t: number, px: number, pz: number): void {
    ...
    if (r.timer <= 0) {
        switch (r.state) {
            case 'idle':
                r.state = 'hop';
                r.timer = 0.6 + Math.random() * 0.6;
                r.dir = Math.random() * Math.PI * 2;
                r.hopT = 0;
                r.tx = r.group.position.x + Math.cos(r.dir) * (1.5 + Math.random() * 2);
                r.tz = r.group.position.z + Math.sin(r.dir) * (1.5 + Math.random() * 2);
                r.tx = Math.max(r.homeX - r.range, Math.min(r.homeX + r.range, r.tx));
                r.tz = Math.max(r.homeZ - r.range, Math.min(r.homeZ + r.range, r.tz));
                break;
            case 'hop':
                r.state = 'idle';
    ...
    if (r.state === 'hop' || r.state === 'flee') {
        ...
        if (r.state === 'flee') {
            r.tx = r.group.position.x + Math.cos(r.dir) * spd * dt;
            r.tz = r.group.position.z + Math.sin(r.dir) * spd * dt;
            r.tx = Math.max(r.homeX - r.range * 1.5, Math.min(r.homeX + r.range * 1.5, r.tx));
            r.tz = Math.max(r.homeZ - r.range * 1.5, Math.min(r.homeZ + r.range * 1.5, r.tz));
        }
...
function createBird(scene: THREE.Scene, x: number, y: number, z: number, color: number): Bird {
    ...
    g.add(at(bx(0.18, 0.16, 0.32, color), 0, 0, 0));
    g.add(at(bx(0.14, 0.10, 0.22, BELLY), 0, -0.04, 0.02));

    const head = new THREE.Group();
    head.position.set(0, 0.10, 0.16);
    head.add(bx(0.15, 0.14, 0.15, color));
    head.add(at(bx(0.05, 0.04, 0.08, BEAK), 0, -0.02, 0.10));
    const eL = fp(0.03, 0.04, DARK); eL.rotation.y = -0.3;
    head.add(at(eL, -0.065, 0.02, 0.055));
    const eR = fp(0.03, 0.04, DARK); eR.rotation.y = 0.3;
    head.add(at(eR, 0.065, 0.02, 0.055));
    g.add(head);

    const wingL = at(bx(0.26, 0.03, 0.16, color), -0.18, 0.02, -0.02);
    const wingR = at(bx(0.26, 0.03, 0.16, color), 0.18, 0.02, -0.02);
    g.add(wingL, wingR);

    const tail = at(bx(0.10, 0.03, 0.12, color), 0, 0.02, -0.22);
    tail.rotation.x = 0.2;
    g.add(tail);
    ...
    if (b.timer <= 0) {
        switch (b.state) {
            case 'perch':
                ...
                break;
            case 'fly':
                b.state = 'glide'; b.timer = 1.5 + Math.random();
                break;
            case 'glide':
...
function createButterfly(scene: THREE.Scene, x: number, z: number, cols: [number, number]): Butterfly {
    ...
    g.add(at(bx(0.03, 0.03, 0.12, 0x333333), 0, 0, 0));

    const wingL = at(bx(0.14, 0.01, 0.11, cols[0]), -0.08, 0, 0);
    const wingR = at(bx(0.14, 0.01, 0.11, cols[1]), 0.08, 0, 0);
    g.add(wingL, wingR);

    // 날개 점
    const sMat = mt(0xffffff);
    const sGeo = new THREE.BoxGeometry(0.035, 0.015, 0.035);
    g.add(at(new THREE.Mesh(sGeo, sMat), -0.09, 0.01, 0));
    g.add(at(new THREE.Mesh(sGeo, sMat), 0.09, 0.01, 0));

    // 더듬이
    const ant = bx(0.01, 0.01, 0.05, 0x333333);
    const antL = ant.clone(); antL.position.set(-0.02, 0.02, 0.08);
    antL.rotation.set(-0.4, 0, -0.3);
    const antR = ant.clone(); antR.position.set(0.02, 0.02, 0.08);
    antR.rotation.set(-0.4, 0, 0.3);
    g.add(antL, antR);
...
export function createAnimals(scene: THREE.Scene): Animals {
    const rabbits: Rabbit[] = [
        createRabbit(scene, 4, 3),
        createRabbit(scene, -5, 2),
        createRabbit(scene, 6, -16),
        createRabbit(scene, -31, -43),
    ...
    const bfSpawns: [number, number][] = [
        [-5, -3], [7, -8], [10, -20], [-9, -25], [3, -50],
    ];
    const butterflies: Butterfly[] = bfSpawns.map((s, i) =>
