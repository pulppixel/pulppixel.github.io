...
IW Zombie에서 쓰던 채팅 시스템을 메타버스 규모에 맞춰 재구성했습니다. 이 프로젝트에서 VContainer 기반 DI + MVVM을 실제로 적용해 봤는데, 동작 자체는 깔끔했지만 DI 컨테이너의 학습 비용이 팀 전체로 번지고 MVVM과 섞이면서 의존 그래프를 읽기 어려워지는 구간을 관찰했습니다. 여기서 얻은 관점이 이후 ETERNA의 3계층 설계로 이어졌습니다.
